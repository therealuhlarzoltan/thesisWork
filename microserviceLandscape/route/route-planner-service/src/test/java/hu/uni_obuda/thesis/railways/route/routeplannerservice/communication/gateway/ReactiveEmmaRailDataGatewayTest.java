...
class ReactiveEmmaRailDataGatewayTest {

    @Mock
    private EmmaRailWebClient webClient;
    ...
    private ReactiveEmmaRailDataGateway testedObject;

    @BeforeEach
    void setUp() {
        testedObject = new ReactiveEmmaRailDataGateway(
                webClient,
    ...
    void getTimetable_success_returnsClientFlux() {
        ...
        when(webClient.makeRouteRequest(from, fromLat, fromLon, to, toLat, toLon, date))
                .thenReturn(Flux.just(route1, route2));

        Flux<TrainRouteResponse> result =
                testedObject.getTimetable(from, fromLat, fromLon, to, toLat, toLon, date);

        StepVerifier.create(result)
                .expectNext(route1, route2)
                .verifyComplete();

        verify(webClient).makeRouteRequest(from, fromLat, fromLon, to, toLat, toLon, date);
    }
