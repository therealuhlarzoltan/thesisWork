...
public class EmmaRailDataGatewayTest {
    ...
    public void setUp() {
        ...
        testedObject = new EmmaRailDataGatewayImpl(
                webClient,
    ...
    public void getShortTimetable_whenUpstreamSucceeds_thenPassesThrough() {
        ...
        when(webClient.getShortTimetable("FROM", 47.0, 19.0, "TO", 48.0, 20.0, date))
                .thenReturn(Mono.just(response));

        Mono<EmmaShortTimetableResponse> result =
                testedObject.getShortTimetable("FROM", 47.0, 19.0, "TO", 48.0, 20.0, date);

        StepVerifier.create(result)
                .expectNext(response)
                .verifyComplete();

        verify(webClient).getShortTimetable("FROM", 47.0, 19.0, "TO", 48.0, 20.0, date);
    }
    ...
    public void getShortTrainDetails_whenUpstreamSucceeds_thenPassesThrough() {
        ...
        when(webClient.getShortTrainDetails("TRAIN_ID", serviceDate))
                .thenReturn(Mono.just(response));

        StepVerifier.create(testedObject.getShortTrainDetails("TRAIN_ID", serviceDate))
                .expectNext(response)
                .verifyComplete();
    }
    ...
    public void getTimetable_whenUpstreamSucceeds_thenPassesThrough() {
        ...
        when(webClient.getTimetable("FROM", 47.0, 19.0, "TO", 48.0, 20.0, date))
                .thenReturn(Mono.just(response));

        StepVerifier.create(testedObject.getTimetable("FROM", 47.0, 19.0, "TO", 48.0, 20.0, date))
                .expectNext(response)
                .verifyComplete();
    }
    ...
    public void getShortTimetable_whenWebClientResponseException_thenMapsToExternalApiException() {
        ...
        StepVerifier.create(testedObject.getShortTimetable("FROM", 47.0, 19.0, "TO", 48.0, 20.0, date))
